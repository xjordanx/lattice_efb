...
**  3.1    01/23/2025  Updated for Propel SDK and RISC-V 32-bit
**
*****************************************************************************/
#include "stddef.h"
#include "EFB_SPI.h"
...
void EFB_SPIInit(void)
{
	...
	EFB_SPI_WRITE_BR(EFB_WB_ADDR, 0x6);
}
	...
	unsigned char bufferSize)
{
	...
	do 
	{
		EFB_SPI_READ_SR (EFB_WB_ADDR, sts);
		if ((sts & EFB_SPI_SR_TIP) == 0)
			break;
	} while (1);
	...
	EFB_SPI_WRITE_CSR (EFB_WB_ADDR, ~(SPI_SLAVE_INDEX));			
	EFB_SPI_WRITE_CR2 (EFB_WB_ADDR, (EFB_SPI_CR2_MSTR | EFB_SPI_CR2_MCSH /*| EFB_SPI_CR2_CPOL | EFB_SPI_CR2_CPHA */));
	...
	do 
	{
		...
		EFB_SPI_WRITE_TXDR(EFB_WB_ADDR, *txBuffer);
		txBuffer++;
	    ...
	    {
			EFB_SPI_READ_RXDR(EFB_WB_ADDR, *rxBuffer);
			rxBuffer++;
	    }
		else  // just throw away recvd byte
		{
			EFB_SPI_READ_RXDR(EFB_WB_ADDR, dummy);		
		}
	...
	// End transaction
	EFB_SPI_WRITE_CR2(EFB_WB_ADDR, EFB_SPI_CR2_MSTR);  // stop command
						...
						unsigned char data)
{
    ...
    {
        EFB_SPI_READ_SR (EFB_WB_ADDR, sts);
        if ((sts & EFB_SPI_SR_TRDY) != 0) 
        	break;
    } while (1);
    
    // Write byte to transmit register
    EFB_SPI_WRITE_TXDR (EFB_WB_ADDR, data);
						...
						unsigned char *data)
{
    ...
    {
        EFB_SPI_READ_SR (EFB_WB_ADDR, sts);
        if ((sts & EFB_SPI_SR_RRDY) != 0) 
        	break;
    } while (1);
    
    // Get byte from receive register
    EFB_SPI_READ_RXDR (EFB_WB_ADDR, *data);
